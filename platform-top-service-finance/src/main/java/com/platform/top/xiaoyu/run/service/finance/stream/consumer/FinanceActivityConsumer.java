...
@Service
@Slf4j
public class FinanceActivityConsumer {
	...
	public void readMessage(@Header("messageType") String messageType, UserRegistMoney msg) {
		if(UserRegistMoney.USER_REGIST_MONEY != Integer.parseInt(messageType)) {
			return;
		}

		log.info("接收到消息==》 注册送彩金 " + msg.toString());
		if ( null == msg ) {
			log.info("接收到消息==》 注册送彩金 为 null ， 不处理。" + msg.toString());
			return;
		}
		if(null == msg.getId()) {
			log.info("注册送彩金 消息ID 为空， 不处理 ");
			return;
		}
		...
		if (amount <= 0) {
			log.info("接收到消息==》 注册送彩金 为 0 ， 不处理。" + msg.toString());
			return;
		}
