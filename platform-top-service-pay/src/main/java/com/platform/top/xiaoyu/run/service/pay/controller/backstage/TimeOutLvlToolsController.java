...
import com.platform.top.xiaoyu.run.service.api.pay.vo.req.timeout.TimeOutInsertReq;
import com.platform.top.xiaoyu.run.service.api.pay.vo.req.timeout.TimeOutPageReq;
import com.platform.top.xiaoyu.run.service.api.pay.vo.req.timeout.TimeOutUpdateReq;
...
@RestController
@AllArgsConstructor
@RequestMapping(PayConstant.BACKSTAGE_TIMEOUT)
@Api(value = "接口超时等级积分配置", tags = "接口超时等级积分配置")
@BackstageController
@MenuDefine(name = "接口超时等级积分配置", moduleName = "TimeOutLvlTools", parentCode = "payManage")
public class TimeOutLvlToolsController extends TopBaseController {
	...
	public R<PageResp<TimeOutVO>> findPage(@RequestBody @Validated TimeOutPageReq req) {
		if(req.getPage() <= 0 ) {
		...
		Page<TimeOutVO> page = new Page<TimeOutVO>(req.getPage(), req.getSize());
		TimeOutVO vo = BeanCopyUtils.copyBean(req, TimeOutVO.class);
		vo.setPlatformId(SecurityUtil.getLoginAccount().getPlatformId());
		return R.data(new PageResp(timeOutService.findPage(page, vo)));
	}
	...
	public R<TimeOutVO> findDetail(@RequestBody @Validated IdReq req) {
		if(null == req || null == req.getId() || req.getId() == 0) {
			throw new BizBusinessException(BasePayExceptionType.PARAM_ID);
		}
		return R.data(timeOutService.findDetail(req.getId(), SecurityUtil.getLoginAccount().getPlatformId()));
	}
	...
	public R insert(@RequestBody @Validated TimeOutInsertReq req) {
		if( null == req) {
		...
		if( req.getTimeOutSecond().intValue() <= 0 ) {
			throw new BizBusinessException(BasePayExceptionType.PARAM_TIMEOUTSECOND);
		}
		TimeOutVO vo = BeanCopyUtils.copyBean(req, TimeOutVO.class);
		vo.setPlatformId(SecurityUtil.getLoginAccount().getPlatformId());
		return R.data(timeOutService.insert(vo));
	}
	...
	public R update(@RequestBody @Validated TimeOutUpdateReq req) {
		if( null == req) {
			throw new BizBusinessException(BasePayExceptionType.PARAM_NULL);
		}
		...
		TimeOutVO vo = BeanCopyUtils.copyBean(req, TimeOutVO.class);
		vo.setPlatformId(SecurityUtil.getLoginAccount().getPlatformId());
		return R.data(timeOutService.update(vo));
	}
	...
	public R delBatch(@RequestBody @Validated List<Long> ids) {
		return R.data(timeOutService.delBatch(ids, SecurityUtil.getLoginAccount().getPlatformId()));
	}
