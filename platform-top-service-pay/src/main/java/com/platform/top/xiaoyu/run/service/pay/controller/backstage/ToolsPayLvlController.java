...
import com.platform.top.xiaoyu.run.service.api.pay.vo.req.toolspay.ToolsPayAllReq;
import com.platform.top.xiaoyu.run.service.api.pay.vo.req.toolspay.ToolsPayInsertReq;
import com.platform.top.xiaoyu.run.service.api.pay.vo.req.toolspay.ToolsPayPageReq;
import com.platform.top.xiaoyu.run.service.api.pay.vo.req.toolspay.ToolsPayUpdateReq;
...
@RestController
@AllArgsConstructor
@RequestMapping(PayConstant.BACKSTAGE_USERPAYLVL_TOOLS)
@Api(value = "接口等级积分配置", tags = "接口等级积分配置")
@BackstageController
@MenuDefine(name = "接口等级积分配置", moduleName = "ToolsPayLvl", parentCode = "payManage")
public class ToolsPayLvlController extends TopBaseController {
	...
	public R<PageResp<ToolsPayVO>> findPage(@RequestBody @Validated ToolsPayPageReq req) {
		if(req.getPage() <= 0 ) {
		...
		Page<ToolsPayVO> page = new Page<ToolsPayVO>(req.getPage(), req.getSize());
		ToolsPayVO vo = BeanCopyUtils.copyBean(req, ToolsPayVO.class);
		vo.setPlatformId(SecurityUtil.getLoginAccount().getPlatformId());
		return R.data(new PageResp(toolsPayService.findPage(page, vo)));
	}
	...
	public R<List<ToolsPayVO>> findListAll(@RequestBody @Validated ToolsPayAllReq req) {
		ToolsPayVO vo = BeanCopyUtils.copyBean(req, ToolsPayVO.class);
		vo.setPlatformId(SecurityUtil.getLoginAccount().getPlatformId());
		return R.data(toolsPayService.findListAll(vo));
	}
	...
	public R<ToolsPayVO> findDetail(@RequestBody @Validated IdReq req) {
		if(null == req || null == req.getId() || req.getId() == 0) {
			throw new BizBusinessException(BasePayExceptionType.PARAM_ID);
		}
		return R.data(toolsPayService.findDetail(req.getId(), SecurityUtil.getLoginAccount().getPlatformId()));
	}
	...
	public R insert(@RequestBody @Validated ToolsPayInsertReq req) {
		if( null == req) {
		...
		if( req.getNumber().intValue() <= 0 ) {
			throw new BizBusinessException(BasePayExceptionType.PARAM_NUMBER);
		}
		ToolsPayVO vo = BeanCopyUtils.copyBean(req, ToolsPayVO.class);
		vo.setPlatformId(SecurityUtil.getLoginAccount().getPlatformId());
		return R.data(toolsPayService.insert(vo));
	}
	...
	public R update(@RequestBody @Validated ToolsPayUpdateReq req) {
		if( null == req) {
			throw new BizBusinessException(BasePayExceptionType.PARAM_NULL);
		}
		...
		ToolsPayVO vo = BeanCopyUtils.copyBean(req, ToolsPayVO.class);
		vo.setPlatformId(SecurityUtil.getLoginAccount().getPlatformId());
		return R.data(toolsPayService.update(vo));
	}
	...
	public R delBatch(@RequestBody @Validated List<Long> ids) {
		return R.data(toolsPayService.delBatch(ids, SecurityUtil.getLoginAccount().getPlatformId()));
	}
