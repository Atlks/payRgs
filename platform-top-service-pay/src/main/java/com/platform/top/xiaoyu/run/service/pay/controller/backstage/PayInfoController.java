...
import com.platform.top.xiaoyu.run.service.api.pay.vo.req.payinfo.PayInfoPageReq;
...
@RestController
@AllArgsConstructor
@RequestMapping(PayConstant.BACKSTAGE_PAY)
@Api(value = "系统支付", tags = "系统支付")
@BackstageController
@MenuDefine(name = "系统支付", moduleName = "PayInfo", parentCode = "payManage")
public class PayInfoController extends TopBaseController {
	...
	public R<PageResp<PayInfoVO>> findPage(@RequestBody @Validated PayInfoPageReq req) {
		if(req.getPage() <= 0 ) {
		...
		Page<PayInfoVO> page = new Page<PayInfoVO>(req.getPage(), req.getSize());
		PayInfoVO vo = BeanCopyUtils.copyBean(req, PayInfoVO.class);
		vo.setPlatformId( SecurityUtil.getLoginAccount().getPlatformId());
		return R.data(new PageResp(iPayInfoService.findPage(page, vo)));
	}
	...
	public R<PayInfoVO> findDetail(@RequestBody @Validated IdReq req) {
		if(null == req.getId() || req.getId().longValue() <= 0 ) {
			throw new BizBusinessException(BasePayExceptionType.PARAM_ID);
		}
		return R.data(iPayInfoService.findDetailId(req.getId(), SecurityUtil.getLoginAccount().getPlatformId()));
	}
